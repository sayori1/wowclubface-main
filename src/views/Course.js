...
const Course = ({ route, navigation, userDetails }) => {
  ...
  const exercises = course.exercises.map((exercise) => {
    exercise.video = `http://80.78.251.86:3000/video/${exercise.content}?token=${userDetails.token}`;
  ...
  const next = () => {
    if (exercises.length <= index + 1) {
      return navigation.navigate('Done', { courseName: course.name, id: course._id })
    }
          ...
          <View style={styles.progressBars}>
            {
              exercises.map((exercise, exerciseIndex) => {
                if (exerciseIndex < index) {
                  return (
                    <View style={[styles.progressBar, { width: (90/exercises.length) + '%'}]} key={exerciseIndex}>
                      <Animated.View style={{...styles.progressBarContent, ...styles.progressBarContentFilled}}/>
                    </View>
                  )
                }
                ...
                return (
                  <View style={[styles.progressBar, { width: (90/exercises.length) + '%'}]} key={exerciseIndex}>
                    <Animated.View style={{...styles.progressBarContent}}/>
                  </View>
                );
              })
            }
        ...
        <View style={styles.content}>
          ...
          <View style={styles.buttonWrapper}>
            <View style={styles.exerciseData}>
              ...
              {exercises[index].name}: {exercises[index].tip}
              </Text>
            </View>
...

            {
              index > 0 && (
                ...
                >
                  <Text style={{
                    ...styles.buttonTextBack,
                  }}>Предыдущее упражнение</Text>
                </TouchableOpacity>
              )
            }
          </View>
